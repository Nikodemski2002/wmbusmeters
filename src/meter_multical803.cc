    ...
    MeterCommonImplementation(mi, MeterType::MULTICAL803)
{
    ...
    addPrint("energy_forward_gj", Quantity::Text,
             [&](){ return to_string(energy_forward_gj_); },
             "Energy forward.",
             false, true);

    addPrint("energy_returned_gj", Quantity::Text,
             [&](){ return to_string(energy_returned_gj_); },
             "Energy returned.",
             false, true);
...
*/
    ...
    t->addMoreExplanation(offset, " something A (%zu)", energy_forward_gj_);

    extractDVuint32(&t->values, "04FF08", &offset, &energy_returned_gj_);
    t->addMoreExplanation(offset, " something B (%zu)", energy_returned_gj_);
